...
" Version:      3.0.1
" Last Change:  Feb 16, 2021
"
" 2021 Feb 16
" - Update keywords for SAS/STAT 14.3
"
" 2017 Mar 7
"
...
syn keyword sasReserved _data_ _last_ _null_
" Strings
...
syn keyword sasDataStepAutoVariable _n_ _error_ _infile_ _all_ _character_ _numeric_ _temporary_ contained
...
syn match sasDataStepStatement '\v%(^|;)\s*\zs%(attrib|format|informat|input|put)>' display contained contains=sasDataStepStatementKeyword nextgroup=sasFormatContext skipwhite skipnl skipempty
...
syn cluster sasDataStepSyntax contains=sasDataStepAutoVariable,sasDataStepFunction,sasDataStepHashOperator,sasDataStepHashAttribute,sasDataStepHashMethod,sasDataStepControl,sasDataStepStatement
...
" SAS/IML, 14.1
syn keyword sasIMLAutoVariable _all_ _char_ _num_ contained
...
syn match sasIMLFunction '\v<\w+\ze\(' contained contains=sasIMLFunctionName,sasIMLCallRoutineName,sasDataStepFunction nextgroup=sasIMLFunctionContext
syn keyword sasIMLControl abort by do else end endsubmit finish goto if link pause quit resume return run start stop submit then to until while contained
syn keyword sasIMLStatementKeyword append call close closefile create delete display edit file find force free index infile input list load mattrib print purge put read remove replace reset save setin setout show sort store summary use contained
...
syn region sasIML matchgroup=sasSectionKeyword start='\v%(^|;)\s*\zsproc\s+iml>' end='\v%(^|;)\s*%(quit|endsas)>'me=s-1 fold contains=@sasBasicSyntax,sasIMLAutoVariable,sasIMLFunction,sasIMLControl,sasIMLStatement
