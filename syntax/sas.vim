...
syn region sasMacroFunction matchgroup=sasMacroFunctionName start='\v\%\w+\ze\(' end=')'me=s-1 contains=@sasBasicSyntax
syn region sasMacroFunction matchgroup=sasMacroFunctionName start='\v\%q=sysfunc\ze\(' end=')'me=s-1 contains=@sasBasicSyntax,sasDataStepFunction
...
syn region sasDataStepFunction start='\v<%(input|put)\(' end=')' contained contains=@sasBasicSyntax,sasDataStepFunction,sasDataStepFunctionHead,sasFormat
...
syn region sasProc matchgroup=sasSectionKeyword start='\v%(^|;)\s*\zsproc%(\s+\h\w*)=>' end='\v%(^|;)\s*%(data|endsas|proc|quit|run)>'me=s-1 fold contains=@sasBasicSyntax,sasDataStepFunction,sasProcStatement
...
syn region sasODSGraphicsProc matchgroup=sasSectionKeyword start='\v%(^|;)\s*\zsproc\s+%(sgdesign|sgpanel|sgplot|sgrender|sgscatter)>' end='\v%(^|;)\s*%(data|endsas|proc|quit|run)>'me=s-1 fold contains=@sasBasicSyntax,sasDataStepFunction,sasODSGraphicsProcStatement
...
syn region sasGraphProc matchgroup=sasSectionKeyword start='\v%(^|;)\s*\zsproc\s+%(g3d|g3grid|ganno|gareabar|gbarline|gchart|gcontour|gdevice|geocode|gfont|ginside|gkpi|gmap|goptions|gplot|gproject|gradar|greduce|gremove|greplay|gslide|gtile|mapimport)>' end='\v%(^|;)\s*%(data|endsas|proc|run)>'me=s-1 fold contains=@sasBasicSyntax,sasDataStepFunction,sasGraphProcStatement
...
syn match sasProcSQLFunctionHead '\v<\w+\(' display contained contains=sasProcSQLFunctionName,sasDataStepFunctionName
syn region sasProcSQLFunction start='\v<\w+\(' end=')' contained contains=@sasBasicSyntax,sasProcSQLFunction,sasProcSQLFunctionHead
...
syn match sasDS2FunctionHead '\v<\w+\(' display contained contains=sasDS2FunctionName
syn region sasDS2Function start='\v<\w+\(' end=')' contained contains=@sasBasicSyntax,sasDS2Function,sasDS2FunctionHead
...
syn region sasDS2 matchgroup=sasSectionKeyword start='\v%(^|;)\s*\zsproc\s+ds2>' end='\v%(^|;)\s*%(data|endsas|proc|quit|run)>'me=s-1 fold contains=@sasBasicSyntax,sasDS2Function,sasDS2Control,sasDS2Statement
...
syn match sasIMLFunctionHead '\v<\w+\(' display contained contains=sasIMLFunctionName,sasIMLCallRoutineName,sasDataStepFunctionName
syn region sasIMLFunction start='\v<\w+\(' end=')' contained contains=@sasBasicSyntax,sasIMLFunction,sasIMLFunctionHead
