...
syn region sasFunction start='\v<\w+\(' end=')' contains=@sasBasicSyntax,sasFunctionName,sasCallRoutineName
...
syn region sasProc matchgroup=sasSectionKeyword start='\v%(^|;)\s*\zsproc%(\s+\h\w*)=>' end='\v%(^|;)\s*%(data|endsas|proc|quit|run)>'me=s-1 fold contains=@sasBasicSyntax,sasProcStatement
...
syn region sasProcSQLFunction start='\v<\w+\(' end=')' contains=@sasBasicSyntax,sasProcSQLFunction,sasProcSQLFunctionName
...
syn region sasIMLFunction start='\v<\w+\(' end=')' contains=@sasBasicSyntax,sasIMLFunction,sasIMLFunctionName,sasIMLCallRoutineName
